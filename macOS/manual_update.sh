...
# instead of assuming a fixed clone location.
...
git fetch -a
git pull
# If you originally cloned this repo with `sudo git clone` it'll be owned by
# root, and the two git commands above will fail with a permission error.
# Fix that once with:
