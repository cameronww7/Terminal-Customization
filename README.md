...
My personal terminal setup. It gets you the same fast, glyph-rich, git-aware prompt whether you're on Kali, Mint, Fedora, or Windows, plus a handful of quality-of-life plugins on the Linux side.
...
All four end up rendering basically the same prompt. Under the hood, they all point at one shared config file, so if you jump between machines the muscle memory carries over. See [`shared/README.md`](shared/README.md) for how that's wired up.
...
- **VPN awareness on Linux.** A segment on the right shows your `tun0` IP the second you connect to a VPN, and just says `No-VPN` when you're not connected to one.
- **Network status on Linux.** Shows your wifi signal strength when you're on wifi, `WIRED` when you're plugged in over ethernet, and `--` when there's nothing to report.
- **Distro-correct OS icons.** Fedora shows its own blue icon, Kali shows its own icon, Mint shows its own leaf, and anything else falls back to a plain clover. This works correctly even under WSL, where a lot of prompt themes get it wrong (more on that in `shared/README.md` if you're curious why).
- **Fast, modern zsh on Linux.** [Zinit](https://github.com/zdharma-continuum/zinit) loads `zsh-autosuggestions` (ghost-text completion from your history) and `fast-syntax-highlighting` (colors commands as you type), without any of the overhead Oh My Zsh used to add. `zoxide` gives you `z <fuzzy-dir-name>` so you don't have to remember full paths anymore.
- **One config to change, not four.** Edit the prompt once in [`shared/`](shared/README.md) and Kali, Mint, and Fedora all pick it up next time you update.
...
```
                ...
                +-------------------+-------------------+
                |                   |                   |
           Kali/.zshrc         Mint/.zshrc         Fedora/.zshrc
                |                   |                   |
                +---- Zinit + Oh My Posh, same plugins --+

                    Windows/terminal-customization.omp.json
                    a hand-maintained copy, since PowerShell
                    can't read the Linux config live
```

Kali, Mint, and Fedora each have their own `.zshrc` (different package managers, different install scripts), but they all load the exact same Zinit plugins and point at the exact same prompt config. Windows can't share that file directly, so it keeps its own copy that has to be updated by hand when the shared one changes.

## Getting started

Every Linux install works the same way, just swap in your OS folder:
